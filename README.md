...
## Patch evaluation
### Scene-oriented patch evaluation
Scene-oriented patch evaluation for BEVFusion-PKU (`bevfusion2`):
```
CUDA_VISIBLE_DEVICES=2 \
python my_main.py bevfusion2 eval_all \
    --patch_cfg     3 \
    --batch_size    1  \
    --patch_type    rec \
    --obj_type      None \
    --score_tres    0.3 \
    --test_name     <Test name of the generated patch> \
```

### Object-oriented patch evaluation
Object-oriented patch evaluation for DeepInteraction (`deepint`):
```
CUDA_VISIBLE_DEVICES=2 \
python my_main.py deepint eval_all \
    --patch_cfg     4 \
    --batch_size    1  \
    --patch_type    rec \
    --obj_type      Targeted \
...
You can run `python my_main.py -h` to see more options.
